...
TemperatureSensor::TemperatureSensor() {
    ...
    oneWireInterface = owb_rmt_initialize(&rmtDriverInfo,
                                          TEMPERATURE_PIN,
                                          RMT_CHANNEL_1,
                                          RMT_CHANNEL_0);
...
void TemperatureSensor::FindDevices() {
    ...
    while (found) {
        std::string device = TemperatureSensor::CreateStringFromRom(
            searchState.rom_code);
        std::cout << NoDevices << " : " << device << std::endl;
        if (searchState.rom_code == knownDevices[0] ||
            searchState.rom_code == knownDevices[1]) {
    ...
    if (NoDevices != totalDevicesNo) {
        std::cout << "Unwanted devices detected!!! Total NoDevices: "
                  << totalDevicesNo << std::endl;
    }
    std::cout << "Found " << NoDevices << "device"
              << (NoDevices == 1 ? "" : "s") << std::endl;
    if (NoDevices != 2)
        esp_restart();
}
...
void TemperatureSensor::InitializeDevices() {
    for (int i = 0; i < NoDevices; i++) {
        ...
        ds18b20_init(ds18b20_info,
                     oneWireInterface,
                     knownDevices[i]); // associate with bus and device
