...
void InterruptHandler::Start() {
    ...
    gpio_set_direction(OVERRIDE_PIN, GPIO_MODE_INPUT);
    gpio_set_direction(SOURCE_AIR_PIN, GPIO_MODE_INPUT);

    gpio_set_intr_type(ELECTRIC_METER_PIN_1, GPIO_INTR_NEGEDGE);
    gpio_set_intr_type(FORWARD_BUTTON, GPIO_INTR_NEGEDGE);
    gpio_set_intr_type(OVERRIDE_PIN, GPIO_INTR_NEGEDGE);
    gpio_set_intr_type(SOURCE_AIR_PIN, GPIO_INTR_NEGEDGE);
    ...
    gpio_isr_handler_add(OVERRIDE_PIN, OverrideAutomatic, nullptr);
    gpio_isr_handler_add(SOURCE_AIR_PIN, SetAirManually, nullptr);
}
